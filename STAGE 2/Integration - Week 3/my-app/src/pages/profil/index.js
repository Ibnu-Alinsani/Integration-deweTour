...
export default function Profile(props) {
  ...
  const { data: userTransaction } = useQuery("userTransCache", async () => {
    const response = await API.get(`/user/${state.user.id}`)
    return response.data.data
  })

  console.log(user, "ini user")
  console.log(userTransaction, "ini transaction user")
  ...
  const handleSubmit = useMutation( async (e) => {
    ...
    try {
      ...
      const response = await API.patch(`/update-user/${state.user.id}`, image, config)
    } catch (error) {
        ...
        <div className="wrapper-info-profil ">
          ...
          <div className="photo-user">
            ...
            <form className="label-change-photo bg-orange" onSubmit={(e) => handleSubmit(e)}>
              <input type="file" id="chance-photo" name="chance-photo" onChange={handleChange}/>
              <button
